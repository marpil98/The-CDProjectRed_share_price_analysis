...
def download_data(start = "2022-01-01", end = (date.today()-timedelta(1)).strftime("%Y-%m-%d"), ticker = "CDR.WA", train_split = None):
...
def build_model(data,params):    
    ...
    x = Input((lags,5,1))
    out = siec_konwolucyjna(x,warstwy,filtry,strides,kernel_size,aktywacja,pool_fun,pooling_type = 'global',out_act = aktywacja_out,increase_filters =increase_filters,out_units=2)
    model = Model(x,out)
    model.compile(optimizer=Adam(learning_rate=learning_rate, clipnorm=1.0), loss=MeanSquaredError(), metrics=[RootMeanSquaredError()])
    history = model.fit(rec_x,rec_y,validation_split=.2, 
...
if __name__ == "__main__":
    ...
    def objective(trial):
        ...
        params = dict(warstwy=warstwy,
                      filtry=filtry,
                      strides=strides,
                      kernel_size=kernel_size,
                      increase_filters=increase_filters,
                      aktywacja=aktywacja,
                      aktywacja_out=aktywacja_out,
                      pooling=pooling,
                      lags=lags)
        ...
        # x = Input((lags,3,1))
        # out = siec_konwolucyjna(x,warstwy,filtry,strides,kernel_size,aktywacja,pool_fun,pooling_type = 'global',out_act = aktywacja_out,increase_filters =increase_filters,out_units=1)
        # model = Model(x,out)
        # model.compile(optimizer=Adam(learning_rate=learning_rate, clipnorm=1.0), loss=MeanAbsoluteError(), metrics=[RootMeanSquaredError()])
    ...
    study = optuna.create_study(study_name = 'cdpr_both',direction='minimize', storage=storage_name, load_if_exists=True)
